...
(** The abstract data type representing the the game and its state. It includes the game mode, score mode, number of players, the player themselves, scores for each round for each player, current round, 
    and whether or not the game ended. The most important features of game_state is to keep track of the players in the game, keep track of rounds, update the player's score based on the given score calculated in get_data and interface, as well as keeping track of rounds and when the game ends.*)
...
(** [update_player_score game player_index score] updates each [player_index]'s score based on the input [scores].
    We use player_index to find the name of the player based on its index in the player list array.
    Call this function when updating each individual player score with the calculated score after the player's turn*)

val update_round : t -> unit
(** [update_round game round] updates [game]'s current round. Call this function whenever you are moving to the next round *)
